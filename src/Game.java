    ...
    public void gameProcess() {
        if (!isPaused) {
            ...
            this.ball.transparency = Math.abs((int) (Math.sin(this.ball.animationMoment) *
                    (this.ball.MAX_ANIMATION_ALPHA - this.ball.MIN_ANIMATION_ALPHA) +
                    this.ball.MIN_ANIMATION_ALPHA));
            ...
            if (((ball.getX() > GameField.WIDTH - Player.INDENT - Player.WIDTH - ball.getSize() / 2) &
                    (ball.getY() > (player2.getY() - ball.getSize())) & (ball.getY() < player2.getY()
                    + Player.HEIGHT + ball.getSize())) & ball.getX() < GameField.WIDTH - Player.INDENT) {
                ball.bounceToLeft();
            }
